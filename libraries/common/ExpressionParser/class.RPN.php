<?php
...

    {
        ...
        var $_operation = array (
            ...
            '-'    => array ('difference', 2, 2, '_difference'),

            '*'    => array ('multiplication', 3, 2, '_multiplication'),

            '/'    => array ('division', 3, 2, '_division'),

            'r'    => array ('root', 4, 2, '_root'),

            '^'    => array ('power', 4, 2, '_power'),

            'sin'  => array ('sine', 4, 1, '_sin'),

            'cos'  => array ('cosine', 4, 1, '_cos'),

            'tan'  => array ('tangent', 4, 1, '_tan'),

            'asin' => array ('asine', 4, 1, '_asin'),

            'acos' => array ('acosine', 4, 1, '_acos'),

            'atan' => array ('atangent', 4, 1, '_atan'),

            'sqrt' => array ('square root', 4, 1, '_sqrt'),

            'exp'    => array ('power of e', 4, 1, '_exp'),

            'log'  => array ('logarithm', 4, 1, '_log'),

            'ln'   => array ('natural logarithm', 4, 1, '_ln'),

            'E'  => array ('power of 10', 4, 1, '_E'),

            'abs'  => array ('absolute value', 4, 1, '_abs'),

            '!'    => array ('factorial', 4, 1, '_factorial'),

            'pi'   => array ('value of pi', 5, 0, '_const_pi'),

            'e'    => array ('value of e', 5, 0, '_const_e'),

            'mod'    => array ('modulo', 4, 2, '_mod'),
        ...
        function _raiseError ($error) {
            ...
            throw new Exception("RPN ERROR|" . $error);
        }
        ...
        function calculate($input = '', $angle = 'deg', $is_rpn = false) {
        ...
        function _gt($temp, $pos) {

            return $temp[$pos-2] > $temp[$pos-1] ? 1 : 0;

        }

        function _gte($temp, $pos) {

            return $temp[$pos-2] >= $temp[$pos-1] ? 1 : 0;

        }

        function _lt($temp, $pos) {
        ...
        function _lte($temp, $pos) {

            return $temp[$pos-2] <= $temp[$pos-1] ? 1 : 0;

        }

        function _eq($temp, $pos) {
