...
class PBR_Analytics_Btp_Stat_Btp_Request {

	private static $obj;

	public static function getLast($server) {
	    if(!self::$obj)self::$obj = new self($server);
	    return self::$obj;
	}
	...
	public function __construct($server) {
		...
		$this->disabled = PBR_Analytics_Btp_Stat_Btp_Connection::get($server)->isFailed(); //если нет соединения, отключаем сбор счётчиков
		$this->conn = PBR_Analytics_Btp_Stat_Btp_Connection::get($server);
	}
